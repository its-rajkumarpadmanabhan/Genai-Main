...
class SignupRequest(BaseModel):
    ...
    def validate_mobile(cls, v):
        ...
        if not MOBILE_RE.match(v):
            raise ValueError("Enter a valid mobile number (7-15 digits, optional leading +country code).")
        return v
    ...
    def validate_password(cls, v):
        if not PASSWORD_RE.match(v):
            raise ValueError("Password needs 8+ characters with uppercase, lowercase, number, and special character.")
        return v
...
def send_password_reset_email(user: User, reset_token: str):
    ...
    <div style="font-family: Arial, sans-serif; max-width: 560px; margin: auto; background:#0f172a; color:#e2e8f0; padding: 32px; border-radius: 16px;">
      ...
      <a href="{reset_link}" style="display:inline-block; margin: 20px 0; background:#0d9488; color:white; padding: 12px 24px; border-radius: 10px;">Reset Password</a>
    </div>
    """
    send_email(user.email, subject, html)
