  ...
  async function refreshFor(lat, lon, accuracy){
    ...
    try{
      ...
      if(err && err.authRequired){
        ...
        loginMsg.innerHTML = 'Your session is missing or expired — <a href="/login.html">log in again</a>.';
      }else{
  ...
  searchInput.addEventListener('keydown', async (e) => {
    ...
    try{
      ...
      if(!token){
        ...
        loginMsg.style.display = 'block';
        loginMsg.innerHTML = '<a href="/login.html">Log in</a> to search a location.';
        return;
      }
      stopTracking();
