...
from accounts.models import DoctorProfile, PatientProfile, CaretakerProfile, Appointment
from collections import defaultdict

User = get_user_model()

def seed_upcoming():
    print("=== SEEDING UPCOMING APPOINTMENTS FOR PATIENTS & CARETAKERS ===")
    ...
    # Pre-cache patient assigned caretakers
    pat_caretaker_map = {}
    for p in PatientProfile.objects.filter(assigned_caretaker__isnull=False):
        pat_caretaker_map[p.user_id] = p.assigned_caretaker
    ...
    caretaker_booked_counts = defaultdict(int)
    caretaker_targets = {}
    all_caretakers = list(User.objects.filter(role='caretaker'))
    for c in all_caretakers:
        caretaker_targets[c.id] = random.randint(3, 5)

    apts_to_create = []

    for pat in patients:
        assigned_c = pat_caretaker_map.get(pat.id)
        num_upcoming = random.randint(5, 8)
        target_for_c = caretaker_targets.get(assigned_c.id if assigned_c else 0, 4)

        created_count = 0
        attempts = 0
        max_attempts = num_upcoming * 20
        ...
        while created_count < num_upcoming and attempts < max_attempts:
            ...
            if assigned_doc and assigned_slot:
                ...
                is_by_caretaker = False
                if assigned_c and caretaker_booked_counts[assigned_c.id] < target_for_c:
                    is_by_caretaker = True
                    caretaker_booked_counts[assigned_c.id] += 1

                apts_to_create.append(Appointment(
                    patient=pat,
                    doctor=assigned_doc.user,
                    booked_by='caretaker' if is_by_caretaker else 'patient',
                    caretaker=assigned_c if is_by_caretaker else None,
    ...
    # Caretaker upcoming count verification
    ct_upcoming_counts = Appointment.objects.filter(appointment_date__gt=today, caretaker__isnull=False).values('caretaker_id').annotate(cnt=django.db.models.Count('id'))
    ct_counts_list = [item['cnt'] for item in ct_upcoming_counts]

    ct_min = min(ct_counts_list) if ct_counts_list else 0
    ct_max = max(ct_counts_list) if ct_counts_list else 0
    ct_avg = round(sum(ct_counts_list) / len(ct_counts_list), 2) if ct_counts_list else 0

    print(f"Caretaker-Booked Upcoming Appointments Stats: Min = {ct_min}, Max = {ct_max}, Avg = {ct_avg}")
